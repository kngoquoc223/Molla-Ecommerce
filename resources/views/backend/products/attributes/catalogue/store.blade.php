...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3">
          ...
          @endif
         @php 
          $url=($config['method'] == 'create' ) ? route('attribute.catalogue.store') : route('attribute.catalogue.update', $attribute->id);
        @endphp
                    ...
                    <div class="card shadow mb-4">
                        ...
                        <div class="card-body">
                            <form >
                                <div class="form-row">
                                  @php 
                                  $name=$attribute->name ?? old("name")
                                  @endphp
                                  <div class="form-group col-md-12">
                                    <label  class="control-lable text-left">Tên Thuộc Tính<span class="text-danger">(*)</span></label>
                                    <input name="name" type="text" class="form-control" value="{{old('name', $attribute->name ?? '')}}">
                                  </div>
                                </div>
                                      ...
                                      <div class="funkyradio">
                                          <div class="funkyradio-success">
                                              <input type="radio" name="publish" id="radio1" value="2" {{old('publish',
                                                                (isset($attribute->publish))?$attribute->publish:'') == 2 ? 'checked': ''}}/>
                                              <label for="radio1">Kích hoạt</label>
                                          </div>
                                          <div class="funkyradio-success">
                                              <input type="radio" name="publish" id="radio2" value="1" {{old('publish',
                                                                (isset($attribute->publish))?$attribute->publish:'') == 1 ? 'checked': ''}}/>
